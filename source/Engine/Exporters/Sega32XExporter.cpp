...
// A cartridge header field: fixed width, space padded, truncated if the name is
// longer than the field. Anything outside printable ASCII is dropped, because
// the header is read by a machine that predates the idea of anything else, and
// lower case is folded up because that is the convention every cartridge
// follows.
//
// Getting the width wrong here does not produce a wrong name -- it shifts every
// field after it, and the console reads the wrong bytes for all of them.
PRIVATE STATIC void Sega32XExporter::HeaderField(char* out, size_t width, const char* text) {
    size_t at = 0;

    for (const char* c = text; *c && at < width; c++) {
        ...
        // A quotation mark would close the .ascii string it is being written
        // into and a backslash would escape what follows it.
        if (ch == '"' || ch == '\\')
            ch = ' ';
...
PRIVATE STATIC bool Sega32XExporter::WriteSources(const char* outputPath, Sega32XExportResult* result) {
    ...
    // The cartridge header, so the ROM announces the scene rather than the
    // sample the runtime came from.
    const char* sceneName = Scene::CurrentScene[0] ? Scene::CurrentScene : "HATCH SCENE";

    // Just the file, without its folders or its extension: a header field is
    // 48 characters and "Scenes/Level1.tmx" spends a third of that saying
    // nothing.
    const char* leaf = sceneName;
    for (const char* c = sceneName; *c; c++) {
        if (*c == '/' || *c == '\\')
            leaf = c + 1;
    }

    char title[64];
    StringUtils::Copy(title, leaf, sizeof(title));
    char* dot = strrchr(title, '.');
    if (dot)
        *dot = '\0';

    char domestic[49], international[49], copyright[17], serial[17];
    Sega32XExporter::HeaderField(domestic, 48, title);
    Sega32XExporter::HeaderField(international, 48, title);
    Sega32XExporter::HeaderField(copyright, 16, "(C)HATCH 2026.08");
    Sega32XExporter::HeaderField(serial, 16, "GM 00000000-00");

    snprintf(text, sizeof(text),
        "! Generated by the Hatch Game Engine's 32X exporter.\n"
        "!\n"
        "! The cartridge header. Every field is fixed width and the ones after it\n"
        "! are found by counting, so these lengths are not adjustable: 16 for the\n"
        "! system name, 16 for the copyright, 48 each for the domestic and\n"
        "! international titles, 16 for the serial and checksum, 16 for the input\n"
        ...
        "\t\t.ascii\t\"                                        \"\n"
        "\t\t.ascii\t\"JUE             \"\n",
        copyright, domestic, international, serial);

    snprintf(path, sizeof(path), "%s/sh_src/cart_header.inc", outputPath);
    if (!Sega32XExporter::WriteText(path, text)) {
        snprintf(result->Message, sizeof(result->Message), "Could not write \"%s\".", path);
        return false;
    }
